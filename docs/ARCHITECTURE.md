...
- `WatchScene.tsx` — `WatchModel` (GLB `/models/watch.glb`, fallback `ProceduralWatch`) with a scroll-scrubbed macro push-in through three chapters. The camera stays on the dial's axis the whole way (no orbit, no oblique offset) and stops at z = 2.35, where the dial fills the frame; going closer puts the camera inside the case. Scroll and cursor both wind the hands rather than moving the watch body.
...
- `public/models/robot-full.glb` (128 KB, meshopt + WebP textures): full standing humanoid robot, Y-up, facing +Z, bbox 0.34 × 1.00 × 0.17. Only 4,767 verts, so it is never simplified in CI; ships flat per-face normals, which `RobotFull` welds and re-smooths at load. It has no eyes, and none can be found by probing — a depth sweep across the face returns a smooth surface with a single nose ridge and no concavity. `RobotFull` places them by proportion instead (eye line 45% down the head, pupils half a face half-width out) and casts a ray at the face for depth and surface angle, so each eye lies flush however the head is shaped. Each is layered — seated rim, lit iris ring, pupil, glossy dome, catchlight — because a single emissive blob reads as a lamp, not an eye.
  - **One welded mesh, no skeleton** (verified: a single connected component), so nothing can be posed as shipped and no part can be hidden. `robotArmRig.ts` therefore cuts the head and both arms out of the shell by triangle — upper arm / forearm / hand — using a per-height inboard boundary sampled from the mesh's own x-histogram, because the A-pose means a single vertical plane either slices the shoulder off or leaves the forearm attached. Each cut is capped with a ball joint, plus a wider socket fixed to the torso (the ball travels with the arm and stops covering the hole once it lifts). The head is cut at the narrowest cross-section between the shoulders and the skull — found by measuring the mesh, not by assuming a height, since cutting anywhere else decapitates the shoulders or leaves the head welded to the chest — and pivots there, with a collar sphere sealing the join and the eyes parented to it. **Only the head follows the cursor**; the body just breathes and shifts its weight.
...
- `public/models/watch.glb` (4.3 MB): luxury watch with a curved strap and a **complete skeleton movement of its own** — gears, tourbillon, indices and hands are all in the asset, so do not draw a procedural gear train over it. **The dial faces the camera only at rotation `[Math.PI/2, 0, Math.PI/2]`**; the model is a wrist cuff, so its bounding box is near-cubic (0.86 × 0.87 × 1.01) and the dial axis cannot be inferred from geometry — this was found by rendering the candidates. It is also centred on that bounding box, which includes the strap, so the dial sits ~0.39 left of the origin and needs offsetting back. Its own hands are static: `WatchMovement` overlays running ones at z = 0.8, the depth at which they sit on the dial rather than being swallowed by the crystal or floating clear of the case.
